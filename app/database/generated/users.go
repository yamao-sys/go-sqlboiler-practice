...
var (
	...
	userColumnsWithoutDefault = []string{"name", "email", "password", "created_at", "updated_at"}
...
func (o UserSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "database: unable to insert all from user slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "database: failed to get rows affected by insertall for users")
	}
...
func (o UserSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) (int64, error) {
	...
	if !updateColumns.IsNone() && len(update) == 0 {
		return 0, errors.New("database: unable to upsert users, could not build update column list")
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "database: unable to upsert for users")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "database: failed to get rows affected by upsert for users")
	}
...
// LoadTodosByPage performs eager loading of values by page. This is for a 1-M or N-M relationship.
func (s UserSlice) LoadTodosByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadTodosByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s UserSlice) LoadTodosByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
	for _, chunk := range chunkSlice[*User](s, pageSize) {
		if err := chunk[0].L.LoadTodos(ctx, e, false, &chunk, queryMods(mods)); err != nil {
			return err
		}
	}
	return nil
...
func (s UserSlice) GetLoadedTodos() TodoSlice {
	result := make(TodoSlice, 0, len(s)*2)
	for _, item := range s {
		if item.R == nil || item.R.Todos == nil {
			continue
		}
		result = append(result, item.R.Todos...)
	}
	return result
}
